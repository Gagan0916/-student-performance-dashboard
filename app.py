...
def _source_caption(source):
    return "✨ Generated by Gemini" if source == "gemini" else None
...
def _render_ai_insights(rows, cgpa, cgpa_trend_values, key_prefix):
    ...
    with st.expander("🛠️ Subject Improvement Roadmap", expanded=True, key=f"{key_prefix}_roadmap_exp"):
        st.write(roadmap)
        if _source_caption(roadmap_source):
    ...
    with st.expander("🎯 Suggested Goals", expanded=True, key=f"{key_prefix}_goals_exp"):
        ...
        if _source_caption(goal_suggestion_source):
...
if _source_caption(overview_source):
